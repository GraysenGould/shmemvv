/**
 * @file c_shmem_signal_add.c
 * @brief Unit test for the shmem_signal_add() routine.
 */
...
bool test_c_shmem_signal_add(){
  log_routine("shmem_signal_add()");
  ...
  *signal = 0;
  shmem_barrier_all();
  if (mype == 0) {
    log_info("calling shmem_signal_add() signal=%p, target_pe=%d)",
             signal, target_pe);
    shmem_signal_add(signal, 1, target_pe);
  }
...
bool test_c_ctx_shmem_signal_add(){
  log_routine("shmem_signal_add()");
  uint64_t *signal = (uint64_t *)shmem_malloc(sizeof(uint64_t));
  int mype = shmem_my_pe();
  int npes = shmem_n_pes();
  ...
  *signal = 0;
  ...
  if (mype == 0) {
    log_info("calling shmem_signal_add() signal=%p, target_pe=%d)",
             signal, target_pe);
    shmem_ctx_signal_add(ctx, signal, 1, target_pe);
  }
...
int main(int argc, char *argv[]) {
  ...
  /* Test standard C shmem_signal_add */
  result = test_c_shmem_signal_add();

  reduce_test_result("C shmem_signal_add", &result, false);

  /* Test context-specific variant in C */
  result_ctx = test_c_ctx_shmem_signal_add(); 

  reduce_test_result("C shmem_ctx_signal_add", &result_ctx, false);
