...
bool test_c_ctx_shmem_signal_set(){
  log_routine("shmem_signal_set()");
  uint64_t *signal = (uint64_t *)shmem_malloc(sizeof(uint64_t));
  int mype = shmem_my_pe();
  int npes = shmem_n_pes();
  ...
  *signal = 0;
  ...
  if (mype == 0) {
    log_info("calling shmem_signal_set() signal=%p, target_pe=%d)",
             signal, target_pe);
    shmem_ctx_signal_set(ctx, signal, 1, target_pe);
  }
...
int main(int argc, char *argv[]) {
  ...
  /* Test standard C shmem_signal_set */
  result = test_c_shmem_signal_set();
  reduce_test_result("C shmem_signal_set", &result, false);

  /* Test context-specific variant in C */
  result_ctx = test_c_ctx_shmem_signal_set(); 

  reduce_test_result("C shmem_ctx_signal_set", &result_ctx, false);
