...
bool test_c11_shmem_signal_add(){
...
bool test_c11_ctx_shmem_signal_add(){
  log_routine("shmem_signal_add()");
  uint64_t *signal = (uint64_t *)shmem_malloc(sizeof(uint64_t));
  int mype = shmem_my_pe();
  int npes = shmem_n_pes();
  ...
  *signal = 0;
  ...
  if (mype == 0) {
    log_info("calling shmem_signal_add() signal=%p, target_pe=%d)",
             signal, target_pe);
    shmem_signal_add(ctx, signal, 1, target_pe);
  }
...
int main(int argc, char *argv[]) {
  ...
  /* Test standard C11 shmem_signal_add */
  result = test_c11_shmem_signal_add();

  reduce_test_result("C11 shmem_signal_add", &result, false);

  /* Test context-specific variant in C11 */
  result_ctx = test_c11_ctx_shmem_signal_add(); 

  reduce_test_result("C11 shmem_signal_add with ctx", &result_ctx, false);
